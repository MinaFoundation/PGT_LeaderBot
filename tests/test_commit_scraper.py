...
class TestCommitScraper(unittest.TestCase):
    ...
    async def test_get_user_commits_in_repo(self, mock_get_repo, mock_fetch_commits):
        ...
        result = await get_user_commits_in_repo(
            self.username, self.repo_link, self.since, self.until
        )
    ...
    @patch(
        "github_tracker_bot.commit_scraper.aiohttp.ClientSession.get",
    ...
    async def test_fetch_commits(self, mock_get):
        ...
        async with aiohttp.ClientSession() as session:
            result = await fetch_commits(
                session, "https://api.github.com/repos/UmstadAI/zkAppUmstad/commits"
            )

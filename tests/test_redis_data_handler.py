...
from github_tracker_bot.redis_data_handler import RedisClient, User, AIDecision, DailyContributionResponse
...
class TestRedisClient(unittest.TestCase):
    ...
    def test_save_user_valid(self):
        ...
        self.mock_redis.hset.assert_called_once_with(f"user:{user.user_handle}", user.__dict__)

    def test_save_user_invalid(self):
        user = User(user_handle="test_user", github_name="test_github", repositories="invalid_repos")
    ...
    def test_get_user_exists(self):
        ...
        self.mock_redis.hgetall.return_value = {k.encode(): v.encode() for k, v in user_data.items()}
    ...
    def test_save_decision(self):
        decision = AIDecision(
            ...
            repository="repo1",
            date="2024-01-01",
            response=DailyContributionResponse(username="test_user", date="2024-01-01", is_qualified=True, explanation="Good job")
        )
